...
if (isset($_POST['ordersubmit'])) {
	...
	if (strlen($_SESSION['login']) == 0) {
		...
		foreach ($value as $qty => $val34) {
			...
			#$prid	="select * from orders where quantity= AND id= 204";
			#$row = mysql_fetch_array($prid);
			#$stock= $row["currentInventory"];

			/** Kundan Sawner's Code here to reduce quantity on rejecting orders start */

			$getProductQuery = mysqli_query($con, "SELECT * FROM products WHERE id=$qty");
			// $productsRet=mysqli_fetch_array($query);
			$product = [];
			if (mysqli_num_rows($getProductQuery) > 0) {
				while ($row = mysqli_fetch_array($getProductQuery)) {
					$product = $row;
					$currentInventoryQty = $product['currentInventory'];
					// echo "<pre>",print_r($product),die;
					if ($currentInventoryQty >= trim($val34)) {
						$query	=	mysqli_query($con, "insert into orders(userId,productId,quantity) values('" . $_SESSION['id'] . "','$qty','$val34')");
						$updqpr  = "UPDATE products set currentInventory=$currentInventoryQty-$val34 WHERE id=$qty";
						$updateprq = mysqli_query($con, $updqpr);

						header('location:order-history.php');
						// $query = mysqli_query($con,"UPDATE products INNER JOIN orders AS ord SET products.currentInventory = (products.currentInventory-ord.quantity) WHERE ord.id=ord.id");
					} else {
						echo "<script>alert('Your have selected the maximum quantity than current inventory.')</script>";
					}
				}
			}
			/** Kundan Sawner's Code here end */
		}
	}
}
?>
	...
	<header class="header-style-1">
		...
		<div class="col-xs-12 col-sm-12 col-md-3 logo-holder">
			...
			<div class="logo">
				...
				<h2 style="text-align: center;"> MV Supermarket  Inventory Management</h2>
									...
									<table class="table table-bordered">
										<thead  style="background-color: black;">
											<tr>
												...
												<th class="cart-sub-total item">Product Code</th>
												<th class="cart-amount">Amount (RM)</th>
											...
											<tr>
												<td colspan="7">
													<div class="shopping-cart-btn">
														<span class="">
															<input style=" background-color: black;" type="submit" name="deleteorder" value="Delete Order" class="btn btn-upper btn-primary pull-right outer-right-xs">

															<input style="  background-color: black;" type="submit" name="ordersubmit" value="Submit Order" class="btn btn-upper btn-primary pull-right outer-right-xs">
														</span>
										...
										<tbody>
											...
											if (!empty($query)) {
												while ($row = mysqli_fetch_array($query)) {
													...
													$subtotal = $_SESSION['cart'][$row['id']]['quantity'] * $row['amount'];
													$totalprice += $amount;
						    ...
						    <img src="admin/productimages/<?php echo $row['id']; ?>/<?#php echo $row['productImage1'];?>" alt="" width="114" height="146">
														...
														<td class="cart-product-name-info">
															...
															<div class="row">
																...
																<div class="col-sm-8">
																	<?php $rt = mysqli_query($con, "select * from producs ");
														...
														<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "" . " " . $row['productCode']; ?></span></td>
														<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "" . " " . $row['amount']; ?></span></td>
						</div>
						</td>
